...
function clashes(dependencies) {
  ...
  for (var i=0; i<dependencies.length; ++i) {
    ...
    var dependency = Package.fromString(dependencies[i]);

    //check the package isn't already used
    if (Object.keys(used).indexOf(dependency.name) !== -1) {
      throw new Error('Cannot depend on two versions of the same package: '+used[dependency.name]+' and '+dependency.toString());
    }

    //remember the package
    used[dependency.name] = dependency.toString();
  }
